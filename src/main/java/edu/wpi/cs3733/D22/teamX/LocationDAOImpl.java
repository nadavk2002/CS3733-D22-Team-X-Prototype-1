...
// in theory alot of the functionallity of this thing is handled in userProgram but cannot be used
// because of the private flag (3/27/22)
public class LocationDAOImpl implements LocationDAO {
  ...
  public void updateLocation(Location location) {
    LinkedList<Location> newLocationList =
        new LinkedList<Location>(); // location list being updated.
    ...
    for (int i = 0;
        i < locations.size();
        i++) { // check if exits correctly [0.1,2,3].size = 4 exit when i = 3
    ...
    try {
      // create the statement
      Statement statement = connection.createStatement();
      // execute query to see if location even exists? something else needs to be done maybe? (throw
      // exception?)
      statement.executeQuery(
          "SELECT * FROM Location WHERE nodeID = '" + location.getNodeID() + "'");
